 ...
 * <p>
 * For showing the result of each phase, the users can call their
 * desired method in the main method <code> public static void
...
public class MainMethods {
	...
	public static void DataConverter() {
		...
		persons = DataConverterUtils.convertMapWithKeyString(ParsingCsvFile.personData());
		stores = DataConverterUtils.convertMapWithKeyString(ParsingCsvFile.storeData());
		items = DataConverterUtils.convertMapWithKeyString(ParsingCsvFile.itemData());
	...
	/**
	 * This is the result of the Phase 2, which requires to create three reports, but loading data from CSV file:
	 * <li>Summary Report;
	 * <li>Store Sales Summary Report;
	 * <li>Detail Report.
	 * <p>
	 * For seeing the detail of formating those report, searching in {@link InvoiceReportFormating}.
	 */
	public static void InvoiceReportFromCSV() {
		...
		List<Item> listInvItem = ParsingCsvFile.invoiceItemData(hashMapInvoice);

		// Call out the method to convert the HashMap to the List
		List<Store> listStore = DataConverterUtils.convertMapWithKeyString(hashMapStore);
		List<Invoice> listInvoice = DataConverterUtils.convertMapWithKeyString(hashMapInvoice);
	...
	/**
	 * This is the result of the Phase 4, which requires to create three reports, but loading data from Database:
	 * <li>Summary Report;
	 * <li>Store Sales Summary Report;
	 * <li>Detail Report.
	 * <p>
	 * For seeing the detail of formating these reports, searching in {@link InvoiceReportFormating}.
	 */
	public static void InvoiceReportFromDBD() {
		...
		@SuppressWarnings("unused")
		List<Item> listInvItem = DatabaseLoader.getItemData(hashMapInvoice);

		// Call out the method to convert the HashMap to the List
		List<Store> listStore = DataConverterUtils.convertMapWithKeyInteger(hashMapStore);
		List<Invoice> listInvoice = DataConverterUtils.convertMapWithKeyInteger(hashMapInvoice);
